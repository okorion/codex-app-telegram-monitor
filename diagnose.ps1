...
function Get-RedactedEnvKeys {
    return @(
        ...
        "CODEX_POLLING_CONFLICT_STALE_SECONDS",
        "CODEX_GROUP_UNKNOWN_COMMAND_SHOW_HELP"
    )
}
...
$recentLogRawLines = if ($logExists) { @(Get-Content -LiteralPath $logFile -Tail 100 -Encoding UTF8) } else { @() }
$lastLogLines = if ($logExists) { @($recentLogRawLines | Select-Object -Last 10 | ForEach-Object { ConvertTo-CodexRedactedText -Text $_ }) } else { @() }
$pollingConflictStaleSeconds = Get-CodexPollingConflictStaleSeconds
$pollingConflictDetected = Test-CodexRecentTelegramConflict -Lines $recentLogRawLines -StaleSeconds $pollingConflictStaleSeconds
